...
int index_load(Index *index) {
    ...
    if (!f) {
        // No index file yet — fresh repo, not an error
        return 0;
    }
...
int index_save(const Index *index) {
    ...
    // Flush → sync → close → atomic rename
    fflush(f);
    fsync(fileno(f));
    fclose(f);
    free(sorted);
